...
/* ──────────────────────────────────────────────────────────────
 * 4. ONE-CLICK APPROVAL HANDLER
 *    Admin clicks the link in the email → user is approved.
 * ──────────────────────────────────────────────────────────────*/
add_action('admin_post_whr_approve', function () {

    $user_id = absint($_GET['user_id'] ?? 0);
    $nonce   = sanitize_text_field($_GET['nonce'] ?? '');

    if (! $user_id || ! wp_verify_nonce($nonce, 'whr_approve_' . $user_id)) {
    ...
    // Promote to wholesale_customer
    $user->set_role('wholesale_customer');
    delete_user_meta($user_id, 'whr_pending');

    // Notify the applicant
    whr_notify_user_approved($user_id);

    // Redirect admin back to the Users list with a success notice
    wp_redirect(add_query_arg(
        ['whr_approved' => '1', 'user' => $user_id],
        admin_url('users.php')
    ));
    exit;
});
...
/* ──────────────────────────────────────────────────────────────
 * 5. ADMIN NOTICE AFTER APPROVAL
 * ──────────────────────────────────────────────────────────────*/
add_action('admin_notices', function () {
    if (isset($_GET['whr_approved']) && $_GET['whr_approved'] === '1') {
        $user_id = absint($_GET['user'] ?? 0);
        $user    = get_userdata($user_id);
        $name    = $user ? esc_html($user->display_name) : 'User';
        echo '<div class="notice notice-success is-dismissible"><p>'
            . "<strong>{$name}</strong> has been approved as a Wholesale Customer and notified by email."
            . '</p></div>';
    }
});
...
/* ──────────────────────────────────────────────────────────────
 * 6. APPROVAL EMAIL TO THE USER
 * ──────────────────────────────────────────────────────────────*/
function whr_notify_user_approved($user_id)
{
    ...
    $subject = "Your {$site_name} wholesale account has been approved!";

    $message  = "Hi {$user->first_name},\n\n";
    $message .= "Great news — your wholesale account application has been approved.\n\n";
...
add_action('init', function () {
    if (isset($_GET['check_user']) && current_user_can('administrator')) {
        $u = get_userdata(intval($_GET['check_user']));
        wp_die(print_r($u->roles, true));
    }
});
